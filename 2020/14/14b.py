...
instructions = ["mask = 011011X11X11100101XX0XX0100100000X0X",
                "mem[48514] = 171994",
                "mem[14856] = 472531",
                "mem[57899] = 15860",
                "mem[41284] = 37917047",
...
def mask_address(address:int, mask:str):
    new_address_code = ""
    curr_bit_value = "{0:036b}".format(address)
    for value_bit, mask_bit in zip(curr_bit_value, mask):
        if mask_bit == "0":
            new_address_code += value_bit
        else:
            new_address_code += mask_bit
    return new_address_code

def get_all_address_combinations(floating_address):
    all_floating_indexes = [i for i, x in enumerate(floating_address) if x == "X"]
    floating_elements = len(all_floating_indexes)
    zero_one_combinations = itertools.product("01", repeat=floating_elements)
    addresses_generated = []

    for combination in zero_one_combinations:
        new_address = floating_address
        for z_o_index, adr_index in enumerate(all_floating_indexes):
            new_address = new_address[:adr_index] + combination[z_o_index] + new_address[adr_index+1:]
        addresses_generated.append(int(new_address, 2))

    return addresses_generated

def mask_memory(instructions):         
    curr_mask = ""
    new_memory = dict()
    ...
    for line in instructions:
        ...
        if instruction == "mask":
            ...
            memory_address = re.search(r'\d+', instruction).group() 
            floating_address = mask_address(int(memory_address), curr_mask)
            addresses_generated = get_all_address_combinations(floating_address)
            for address in addresses_generated:
                new_memory["mem[" + str(address) + "]"] = int(value)
