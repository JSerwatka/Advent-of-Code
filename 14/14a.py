...
def mask_number(number:int, mask:str):
    new_bit_value = ""
    curr_bit_value = "{0:036b}".format(number)
    for value_bit, mask_bit in zip(curr_bit_value, mask):
        if mask_bit == "0":
            ...
            new_bit_value += value_bit
    return int(new_bit_value, 2)
...
def mask_memory(instructions):         
    ...
    for line in instructions:
        ...
        if instruction == "mask":
            ...
            memory_address = "mem[" + re.search(r'\d+', instruction).group() + "]"
            new_value = mask_number(int(value), curr_mask)
            new_memory[memory_address] = new_value
