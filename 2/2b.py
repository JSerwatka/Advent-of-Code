...
def is_password_valid(password_pattern):
    reg_min = re.compile("\d+(?=\-)")
    reg_max = re.compile("(?<=\-)\d+")
    reg_letter = re.compile("\S+(?=:)")
    reg_pass = re.compile("(?<=: )\S+")

    min_val = reg_min.findall(password_pattern)[0]
    max_val = reg_max.findall(password_pattern)[0]
    letter = reg_letter.findall(password_pattern)[0]
    password = reg_pass.findall(password_pattern)[0]

    found_letters = [password[int(min_val)-1], password[int(max_val)-1]]
    
    if (found_letters[0] == letter) != (found_letters[1] == letter):
        return True

password_patterns = ["6-7 z: dqzzzjbzz",
...
for password_pattern in password_patterns:
    if is_password_valid(password_pattern):
        counter += 1
